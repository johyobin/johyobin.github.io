window.techEvolutionMap = (() => {
  ...
  const nodes = [
    ...
    N('html','web','1995','HTML과 URL','HTML / URL','frontend','웹 문서를 서로 다른 시스템에서 같은 방식으로 교환할 형식이 필요했다.','문서 구조와 주소 체계를 표준화했다.','브라우저 기반의 보편적 클라이언트가 확산됐다.','정적 자산의 캐시·무결성·접근성을 배포 품질에 포함한다.','브라우저, CDN','표현 형식의 상호운용성이 플랫폼 효과를 만든다.','html'),
    N('relational-data','foundations','1970','관계형 데이터 모델','Relational data model','database','애플리케이션이 저장 형식에 직접 묶이면 변경 비용이 커졌다.','관계와 연산으로 데이터 독립성을 추구했다.','트랜잭션 기반 웹 애플리케이션의 신뢰 가능한 상태 저장소가 됐다.','백업·복구 목표와 쿼리 지연을 제품 요구와 연결한다.','SQL, RDBMS','정합성과 질의 유연성은 스키마 경계를 요구한다.','relational'),
    N('cgi','foundations','1993','동적 서버 처리','CGI','backend','정적 문서만으로는 사용자별 계산과 데이터 조회를 제공할 수 없었다.','웹 서버가 외부 프로그램을 호출해 동적 응답을 생성하게 했다.','애플리케이션 서버와 백엔드 계층으로 이어졌다.','요청당 자원 사용량과 timeout이 곧 용량 계획이 된다.','웹 서버, 애플리케이션 런타임','웹 경계와 업무 로직을 분리할 필요를 드러냈다.','cgi'),
    ...
    N('monolith','web','1995','모놀리스','Monolithic architecture','application','초기 웹 애플리케이션은 작은 팀이 한 제품을 빠르게 만들고 배포할 단순한 단위가 필요했다.','핵심 동작을 하나의 프로세스와 대체로 하나의 배포 단위에 모아 전달 경로를 단순화했다.','계층 분리와 이후 독립 배포 서비스의 비교 기준이 됐다.','전체 배포·확장의 영향 범위와 모듈 경계를 함께 확인한다.','single deployable, modular monolith','모놀리스는 계층이 없다는 뜻이 아니다. 한 배포 단위 안에서도 논리적 경계를 유지할 수 있다.','monolith'),
    N('n-tier','web','1995','계층형 애플리케이션','N-tier / 3-tier','application','UI·업무 로직·데이터 접근이 한곳에 섞이면 변경 영향과 보안 경계가 흐려졌다.','표현·업무·데이터 책임을 논리 계층으로 나누고 필요하면 물리 티어로 분리했다.','역할별 확장과 서비스 분해를 위한 경계 언어를 제공했다.','티어 간 지연, 장애 전파, 연결 풀과 데이터 계층 병목을 관측한다.','load balancer, application server, RDBMS','계층(layer)은 논리 책임, 티어(tier)는 물리 배치다. 물리 분리는 격리를 주지만 네트워크 비용을 늘린다.','n-tier'),
    N('gfs','web','2003','분산 파일 시스템','Google File System','database','대량 데이터와 저가 서버의 빈번한 장애를 단일 파일 시스템이 감당하기 어려웠다.','장애가 흔한 클러스터를 전제로 대용량 파일을 분산 저장했다.','분산 데이터 처리와 객체 스토리지의 설계 기준을 제시했다.','복제, 재조정, 복구 시간을 데이터 내구성 SLO로 둔다.','분산 스토리지','장애를 예외가 아닌 입력으로 설계한다.','gfs'),
    N('mapreduce','web','2004','병렬 데이터 처리','MapReduce','database','대규모 데이터를 병렬 처리하려면 분할·스케줄링·장애 처리를 매번 구현해야 했다.','런타임이 그 반복 작업을 맡도록 추상화했다.','배치 분석과 데이터 플랫폼을 대중화했다.','큐 지연, 처리량, 실패 재시도 비용을 워크로드별로 본다.','batch scheduler, data pipeline','계산 모델의 제약은 운영 복잡성을 플랫폼으로 옮긴다.','mapreduce'),
    ...
    N('paxos','web','1998','분산 합의','Paxos','network','장애와 지연이 있는 여러 노드가 하나의 순서에 동의해야 했다.','비동기 분산 시스템의 합의 알고리즘을 설명했다.','리더 선출과 강한 정합성 제어 평면의 기반이 됐다.','쿼럼 손실, 리더 변경, 복구 시간을 운영 시나리오로 훈련한다.','consensus store','강한 정합성은 가용성과 지연의 비용을 수반한다.','paxos'),
    ...
    N('virtualization','web','2003','가상화','Virtualization','infrastructure','물리 서버 한 대를 한 워크로드가 독점하면 자원 활용과 격리가 모두 나빴다.','VMM으로 여러 OS가 물리 자원을 공유하게 했다.','탄력적 IaaS와 격리된 실행 환경을 가능하게 했다.','과할당, noisy neighbor, 이미지 수명주기를 관리한다.','hypervisor, VM image','추상화 계층은 활용률을 높이지만 관측 경계도 한 층 늘린다.','xen'),
    N('iaas','distributed','2006','탄력적 클라우드','IaaS / EC2','infrastructure','서버 조달과 용량 확보는 느리고 초기 비용이 컸다.','필요할 때 가상 서버를 확보하고 사용량 기반으로 지불하게 했다.','자동 확장과 API 기반 인프라 운영의 전제가 됐다.','용량·비용·계정 경계를 코드와 태그로 관리한다.','EC2, autoscaling','소유에서 소비로 바뀌면 거버넌스와 비용 가시성이 중요해진다.','ec2'),
    N('config-management','web','2005','구성 관리','Configuration management','infrastructure','서버를 수동으로 맞추면 환경 간 차이와 재현 불가능성이 누적됐다.','원하는 상태를 선언해 반복 적용하는 모델을 제공했다.','불변 인프라와 IaC의 운영 습관을 만들었다.','drift, 적용 실패, 비밀값 노출을 지속적으로 점검한다.','Puppet, Ansible','선언은 절차를 숨기지만 현재 상태 관측이 필요하다.','puppet'),
    ...
    N('cicd','distributed','2010','지속적 전달','CI/CD','devops','릴리스가 드물고 수동이면 변경 위험이 한 번에 쌓였다.','언제든 배포 가능한 상태를 유지하는 자동 검증·전달 규율을 만들었다.','작은 변경, 빠른 피드백, 배포 자동화를 가능하게 했다.','리드 타임, 변경 실패율, 롤백 가능성을 파이프라인 품질로 둔다.','CI, CD, artifact registry','배포 빈도는 자동화만이 아니라 검증과 복구 설계에 달렸다.','cicd'),
    ...
    N('kubernetes','cloud','2014','컨테이너 오케스트레이션','Kubernetes','infrastructure','컨테이너 수가 늘자 배치·복구·네트워킹을 사람이 관리할 수 없었다.','선언적으로 컨테이너 워크로드와 서비스를 조정하는 플랫폼을 제공했다.','클라우드 네이티브 운영과 플랫폼 엔지니어링의 기반이 됐다.','조정 지연, 리소스 요청, Pod 복구, 제어 평면 가용성을 본다.','Kubernetes','조정 루프는 원하는 상태와 실제 상태의 차이를 지속적으로 줄인다.','kubernetes'),
    N('gitops','cloud','2017','선언형 배포','GitOps','devops','배포 변경의 승인·추적·실제 상태가 분리돼 운영 판단이 어려웠다.','Git의 버전·불변성과 자동 적용·지속 조정을 결합했다.','감사 가능한 배포와 drift 복구를 가능하게 했다.','sync 실패, drift, 변경 주체와 롤백 경로를 운영한다.','Argo CD, Flux','선언형 desired state는 책임 경계를 명확히 해야 안전하다.','gitops'),
    N('service-mesh','cloud','2017','서비스 메시','Service mesh','network','마이크로서비스마다 보안·재시도·관측을 구현하면 정책이 흩어진다.','서비스 간 통신 계층에 보안·관측·트래픽 제어를 제공했다.','일관된 통신 정책과 세밀한 트래픽 전환이 가능해졌다.','프록시 오버헤드, 인증서, retry 폭주를 반드시 관측한다.','Istio, mTLS','공통 기능을 추출하면 앱은 단순해지지만 플랫폼은 복잡해진다.','mesh'),
    ...
    N('slo','cloud','2016','서비스 수준 목표','SLO','devops','인프라 지표만으로는 사용자가 느끼는 품질과 투자 우선순위를 정하기 어려웠다.','사용자 관점 SLI에 목표값을 둬 신뢰성을 측정 가능하게 했다.','운영과 제품이 공통 품질 언어로 협의할 수 있게 했다.','SLI 정의·측정 창·알림을 고객 영향과 함께 검토한다.','SLO tooling','좋은 목표는 측정 가능한 사용자 경험에서 출발한다.','slo'),
    N('error-budget','cloud','2016','오류 예산','Error budget','devops','안정성과 기능 출시가 감정적 우선순위 싸움이 되기 쉬웠다.','허용 가능한 비신뢰성의 예산으로 출시 속도와 위험을 함께 판단했다.','변경 관리에 객관적 중단·투자 기준을 제공했다.','burn rate, 배포 동결 기준, 복구 작업 우선순위를 합의한다.','SLO dashboards','신뢰성은 100%가 아니라 명시적 위험 선택이다.','budget'),
    ...
    N('llm-agents','cloud-native','2022','LLM 기반 에이전트','LLM agents / ReAct','ai','복잡한 목표를 정해진 워크플로만으로 처리하면 예외와 탐색에 약했다.','추론 흔적과 행동을 번갈아 생성해 외부 환경과 상호작용하는 방식을 제시했다.','도구를 조합해 작업을 수행하는 에이전트 설계가 확산됐다.','프롬프트·도구 권한·실행 흔적을 감사 가능한 신호로 남긴다.','LLM, agent runtime','자율성은 결정 범위를 넓히므로 경계와 검증을 먼저 설계한다.','react'),
