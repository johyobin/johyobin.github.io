...
비용을 줄이기 위해 ECS on EC2의 고밀도 배치를 선택한 뒤, JVM 서비스의 동시 기동과 시스템 프로세스용 메모리 미예약이 겹치며 호스트 메모리 압박이 발생했다. EC2와 Fargate의 비교 기준을 인스턴스 단가가 아닌, 피크와 실패 예산까지 포함한 실제 사용 가능 용량의 총비용으로 다시 잡았다.

- **문제**: 동시 기동 뒤 호스트 메모리 압박과 ECS·SSM agent 연결 단절이 관측됐고, 태스크 재배치 지연으로 서비스 전반에 504 발생
- **판단**: `ECS_RESERVED_MEMORY`, JVM 메모리 사용률, 태스크 배치, ASG 최소 용량을 별도 운영 가정으로 분리하고, EC2와 Fargate는 같은 가용성 목표의 총비용으로 비교
