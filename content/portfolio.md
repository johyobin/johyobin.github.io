...
### [ECS on EC2는 정말 Fargate보다 저렴했을까 — JVM 기동 피크가 바꾼 비용 계산](/posts/jvm-memory-usage-not-load/)

비용을 줄이기 위해 ECS on EC2의 고밀도 배치를 선택한 뒤, 여러 JVM 서비스의 동시 기동으로 호스트 메모리 압박·agent 연결 단절·전면 504가 이어진 사례다. EC2와 Fargate의 비교 기준을 인스턴스 단가가 아닌, 피크와 실패 예산까지 포함한 실제 사용 가능 용량의 총비용으로 다시 잡았다.

- **문제**: 동시 기동 뒤 호스트 메모리 압박과 ECS·SSM agent 연결 단절이 관측됐고, 태스크 재배치 지연으로 서비스 전반에 504 발생
- **판단**: JVM 메모리 사용률·태스크 배치·ASG 최소 용량을 하나의 문제가 아닌 별도 운영 가정으로 분리하고, EC2와 Fargate는 같은 가용성 목표의 총비용으로 비교
- **결과와 한계**: CPU 기반 신호와 인스턴스 분산 배치로 조정 뒤 동일 패턴은 관찰 범위에서 재발하지 않았으나, 당시 호스트 로그 부재로 OOM Killer의 직접 원인은 확정하지 않음
