...
draft = false
weight = 10
summary = 'ECS on EC2 고밀도 배치에서 시스템용 메모리와 예비 용량을 다시 계산한 사례.'
landmarks = ['runtime', 'reliability']
featured = true
+++

비용을 줄이기 위해 ECS on EC2 고밀도 배치를 선택했다. 하지만 시스템 프로세스용 메모리를 태스크 배치 용량에서 제외하지 않은 상태에서 동시 기동이 겹쳤다. 이후 EC2와 Fargate를 인스턴스 단가만이 아니라 시스템용 메모리, 실패 예산, 실제 사용 가능 용량까지 포함해 비교했다.
