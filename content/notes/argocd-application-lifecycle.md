...
summary = 'Git 경로 삭제가 ApplicationSet, Application, child resource 삭제로 이어지는 조건을 분리해 확인합니다.'
landmarks = ['argocd', 'gitops', 'kubernetes']
featured = false
aliases = ['/posts/argocd-application-lifecycle/']
+++
...
이 체크 순서는 대표 글의 controller 경계를 실제 삭제 판단으로 옮긴 것이다. 생성·갱신·drift까지 포함한 전체 흐름은 [Argo CD GitOps에서 애플리케이션 생명주기를 설계하는 법](/notes/argocd-application-lifecycle-design/)에서 이어서 볼 수 있다.
